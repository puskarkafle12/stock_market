...
user_input_moving_avg = st.number_input("Enter 'Five-day Moving Average':", min_value=0.0, step=0.01, value=default_moving_avg)
...
input_sequence = np.concatenate((np.zeros(x_data[-1][1:].shape), user_input_features)).reshape(1, x_data[0].shape[0], x_data[0].shape[1])
prediction = model.predict(input_sequence)
predicted_price = scaler_target.inverse_transform(prediction)[0][0]

st.markdown(f"Predicted Closing Price for {selected_stock} using {model_type}: **:red[Rs. {predicted_price:.2f}]**")